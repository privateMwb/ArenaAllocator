...
`packaging/recipes/arenapro/all/conanfile.py`). Scoped to what isn't obvious
...
- **The release tarball's folder name comes from the GitHub repo
  name, which may not match the CMake project/library name
  (`ArenaPro`).** `cd` into whatever the tarball actually produced,
  not the library name. If you don't have a published release yet,
  use the local repo checkout directly instead (see step 1 below) —
  no need to cut a release just to verify packaging still works.
...
```bash
find $HOME/staging/lib/cmake/ArenaPro -name "*.cmake"
```

Expect `ArenaProConfig.cmake`, `ArenaProConfigVersion.cmake`,
`ArenaProTargets.cmake` (plus a per-config
`ArenaProTargets-<config>.cmake`) under `lib/cmake/ArenaPro/`.
...
target_link_libraries(test PRIVATE ArenaPro::ArenaPro)
EOF
...
just default-constructs a `rain::Arena` and returns, so success
...
```bash
conan create packaging/recipes/arenapro/all --version 1.0.0 \
...
`packaging/recipes/arenapro/all/test_package/` against the freshly-built
package — a `ArenaPro linked and constructed successfully.` line in
