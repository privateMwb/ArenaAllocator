...
static void concurrent_allocs_external_lock() {
    Arena<> arena(4096);
    std::mutex mtx;
...
static void concurrent_frame_cycles_external_lock() {
    Arena<> arena(4096);
    std::mutex mtx;
...
static void run_tests() {
    RUN(concurrent_allocs_external_lock);
    RUN(concurrent_frame_cycles_external_lock);
}
