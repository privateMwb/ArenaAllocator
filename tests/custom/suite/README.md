...
| [Conventions](#conventions) | Registration, assertion, and sanitizer conventions specific to the custom framework |
...
- **Assertions** — `CHK(condition)` checks a condition, and
  `CHK_THROWS(expr, ExceptionType)` checks an exception case. A test may hold
  several `CHK`s.
- **Registration** — every test function is listed with `RUN(name)` in the
  file's `run_tests()`, and the file ends with `REGISTER_TEST_SUITE();`. A
  test that isn't listed in `run_tests()` never runs.
- **Worker threads** — concurrency tests never call `CHK` from a worker
  thread. Each thread records its result in a shared `std::atomic<bool>`,
  and `CHK` checks it on the main thread after every thread has joined.
- **`AP_PRE` violations** — a contract violation can't be tested with
  `CHK_THROWS`: `AP_PRE` is a plain `assert()`, and a failed assert calls
  `abort()`, which can't be caught and would kill the whole test binary.
  Regression tests pin the last valid boundary instead.
- **AddressSanitizer** — a test that relies on `::operator new` throwing
  `std::bad_alloc` for a huge size is compiled out under ASan with
  `ARENA_TESTS_UNDER_ASAN`, and its `RUN()` line sits inside the same `#if`.
- **Moved-from objects** — checks on a moved-from arena are preceded by
