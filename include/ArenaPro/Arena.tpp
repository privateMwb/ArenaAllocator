...
Arena<EnableStats>::Arena(std::size_t size, std::size_t alignment)
    : memory_{allocateMemory(validateSize(size), alignment)}, cap_{size}, offset_{0},
...
template <bool EnableStats> Arena<EnableStats>::~Arena() {
    ...
    if (memory_) {
        ::operator delete(memory_, cap_, std::align_val_t{std::size_t{1} << alignShift_});
    }
}
...
template <bool EnableStats>
constexpr std::size_t Arena<EnableStats>::validateSize(std::size_t size) noexcept {
    AP_PRE(size > 0);
    return size;
}
